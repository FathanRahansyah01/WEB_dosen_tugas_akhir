...
<div class="mb-6">
    ...
    <h1 class="text-2xl font-bold text-gray-900">Edit Catatan Tindak Lanjut</h1>
</div>
...
<div class="bg-white rounded-xl border border-gray-200 shadow-sm p-6 max-w-2xl">
    ...
    <form method="POST" action="{{ route('dosen.tindak-lanjut.update', $followUp) }}">
        @csrf @method('PUT')
        <div class="mb-4">
            ...
            <select id="student_id" name="student_id" required class="w-full px-4 py-2.5 rounded-lg border border-gray-300 focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 outline-none">
                ...
                <option value="{{ $s->id }}" {{ $followUp->student_id == $s->id ? 'selected' : '' }}>{{ $s->nim }} - {{ $s->nama }}</option>
                @endforeach
            </select>
        </div>
        <div class="mb-6">
            <label for="note" class="block text-sm font-medium text-gray-700 mb-1.5">Catatan</label>
            <textarea id="note" name="note" rows="5" required class="w-full px-4 py-2.5 rounded-lg border border-gray-300 focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 outline-none resize-y">{{ old('note', $followUp->note) }}</textarea>
        </div>
